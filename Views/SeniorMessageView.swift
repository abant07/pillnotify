...
// This struct is to allow a medicine taker to communicate with their caretaker
// through push notifications
struct SeniorMessageView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 20) {
                ...
                Text("Send a message to your caretaker")
                    .foregroundStyle(.white)
                    .bold()
                    .shadow(color: .black, radius: 5)
    ...
    // This method retreives the apns token from the medicine taker. Since the medicine taker will contain
    // the apns of the caretaker (look at how getAPN() works), it will then send a push notification to the caretaker
    // using firebase service FCM
    func sendMessage(message: String) {
        let database = Database.database().reference()
        let id = UserDefaults.standard.string(forKey: "usernameKey") ?? "NO ID"
        let userDatabase = database.child("users").child(id)
        var fullname = ""
        var apn = ""
